...
#btn = tk.Button(gui, text ="Cliquez ici!", command = msgCallBack)
#btn.pack()
#gui.mainloop()
#df = pd.read_csv(path, usecols=['Nom','Prénom'], sep=';', header=0)


class Application(TkinterDnD.Tk):
    def __init__(self):
        super().__init__()
        self.title("Détecteur de doublons sous CSV")
...
class SearchPage(tk.Frame):
    ...
    def _display_file(self, event):
        ...
        self.data_table.set_datatable(df.loc[df.duplicated(),:])
        messagebox.showinfo(title="Nombre de doublons détectés",message=df.duplicated().sum())
